...
func main() {
	...
	fmt.Println("-------------- CHANGE CAR COLOR --------------")
	result, err = contract.SubmitTransaction("ChangeCarColor", "CAR1", "yellow")
	if err != nil {
		fmt.Printf("Failed to submit transaction: %s\n", err)
		os.Exit(1)
	}
	...
	_, err = contract.SubmitTransaction("transferOwnership", "CAR4", "OWNER1", "false")
	if err != nil {
	...
	fmt.Println("-------------- GET CAR BY ID --------------")
	result, err = contract.EvaluateTransaction("GetCarById", "CAR4")
	if err != nil {
		fmt.Printf("Failed to evaluate transaction: %s\n", err)
		os.Exit(1)
	}
